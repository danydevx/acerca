<template>
  <MemberLayout>
    <Head :title="`SEO - ${vcard?.name || 'Tarjeta'}`" />

    <PageHeader
      :title="`SEO - ${vcard?.name || 'Tarjeta'}`"
                  ...
                  <div class="alert alert-info">
                    ...
                    <div class="mt-2 p-3 bg-light rounded">
                      <div class="d-flex gap-3">
                        ...
                        <div>
                          <div style="color: #000; font-size: 16px; font-weight: 600;">{{ form.og_title || form.seo_title || vcard?.name }}</div>
            ...
            <div class="tab-pane" :class="{ 'show active': activeTab === 'indexing' }" role="tabpanel">
              <div class="row g-3">
                <div class="col-12">
                  ...
                  />
                  <small class="text-muted d-block mb-3">Si esta desactivado, los motores de busqueda no indexaran tu tarjeta</small>
                </div>
                ...
                <div class="col-12">
                  ...
                  />
                  <small class="text-muted d-block mb-3">Los motores de busqueda seguiran los enlaces de tu tarjeta</small>
                </div>
                ...
                <div class="col-12">
                  ...
                  />
                  <small class="text-muted d-block mb-3">Tu tarjeta aparecera en el mapa del sitio XML</small>
                </div>
                ...
                <div class="col-12 col-md-8">
                  <FieldUrl
                    ...
                    placeholder="https://..."
                  />
                  <small class="text-muted d-block">URL principal de tu tarjeta. Si se deja vacia, se usara la URL de la tarjeta.</small>
                </div>
              </div>
            </div>
            ...
            <div class="tab-pane" :class="{ 'show active': activeTab === 'advanced' }" role="tabpanel">
              <div class="row g-3">
                ...
                <div class="col-12 col-md-6">
                  ...
                  />
                  <small class="text-muted d-block">Selecciona el tipo que mejor describe tu tarjeta</small>
                </div>
              </div>
            </div>
          </div>
...
const form = reactive({
  ...
  schema_enabled: seo.value?.schema_enabled ?? false,
  schema_type: seo.value?.schema_type || 'Person',
  settings: seo.value?.settings || null,
})

const canonicalUrl = computed(() => {
  if (vcard.value?.slug) {
    return `${window.location.origin}/v/${vcard.value.slug}`
  }
  return form.canonical_url || `${window.location.origin}`
})
...
const submit = () => {
  ...
  data.append('schema_type', form.schema_type || 'Person')

  if (form.og_image && typeof form.og_image !== 'string') {
    data.append('og_image', form.og_image)
  }

  router.post(`/member/listings/${listing.value?.id}/vcards/${vcard.value?.id}/seo`, data, {
    onFinish: () => {
      sending.value = false
    },
  })
