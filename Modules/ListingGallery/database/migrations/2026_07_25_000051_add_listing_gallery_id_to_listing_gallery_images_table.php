...
return new class extends Migration
{
    public function up(): void
    {
        $columnExists = collect(DB::select(
            "SHOW COLUMNS FROM listing_gallery_images WHERE Field = ?",
            ['business_gallery_id']
        ))->isNotEmpty();

        $indexExists = collect(DB::select(
            "SHOW INDEXES FROM listing_gallery_images WHERE Key_name = ?",
        ...
        $foreignExists = collect(DB::select(
            ...
            ['listing_gallery_images', 'business_gallery_images_business_gallery_id_foreign']
        ))->isNotEmpty();

        Schema::table('listing_gallery_images', function (Blueprint $table) use ($columnExists, $indexExists, $foreignExists) {
            if (! $columnExists) {
            ...
            if (! $foreignExists) {
                $table->foreign('business_gallery_id')
                    ->references('id')->on('listing_galleries')
                    ->cascadeOnDelete();
            }
    ...
    {
        Schema::table('listing_gallery_images', function (Blueprint $table) {
