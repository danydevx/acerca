...
namespace Modules\ListingFeatures\Http\Controllers\Member;
...
class FeatureController extends Controller
{
    ...
    public function index(Request $request, Listing $business)
    {
        ...
        $businessFeaturesPaginated = ListingFeature::with('feature', 'location')
    ...
    public function store(Request $request, Listing $business)
    {
        ...
        $currentCount = ListingFeature::where('listing_id', $business->id)->count();
    ...
    public function importBulk(Request $request, Listing $business)
    {
    ...
    public function import(Request $request, Listing $business, Feature $feature)
    {
        $this->authorize('import', [Feature::class, $business]);
        ...
        $maxSortOrder = ListingFeature::where('listing_id', $business->id)->max('sort_order') ?? 0;
    ...
    public function update(Request $request, Listing $business, Feature $feature)
    {
    ...
    public function destroy(Request $request, Listing $business, Feature $feature)
    {
        ...
        ListingFeature::where('feature_id', $feature->id)->delete();
        $feature->delete();

        return redirect()->back()->with('success', 'Feature eliminada correctamente.');
    }

    public function updateAssignment(Request $request, Listing $business)
    {
        $user = $request->user();
        ...
        $businessFeature = ListingFeature::where('listing_id', $business->id)
    ...
    public function removeAssignment(Request $request, Listing $business, $assignmentId)
    {
        $user = $request->user();
        if (!$user->hasAnyRole(['superadmin', 'admin']) && $business->user_id !== $user->id) {
        ...
        if ($feature->isClone() || $feature->isCustom()) {
            ListingFeature::where('id', $assignmentId)->delete();
            $feature->delete();
        } else {
            ListingFeature::where('id', $assignmentId)->delete();
        }
    ...
    public function reorder(Request $request, Listing $business)
    {
        ...
        $validated = $request->validate([
            ...
            'order.*' => ['required', 'exists:listing_features,id'],
        ]);

        foreach ($validated['order'] as $index => $id) {
            ListingFeature::where('id', $id)
                ->where('listing_id', $business->id)
