<template>
  <section class="section-services">
    <div class="section-services__inner">
      <header v-if="title || subtitle || description" class="section-services__header">
        <h2 v-if="title" class="section-services__title">{{ title }}</h2>
        <p v-if="subtitle" class="section-services__subtitle">{{ subtitle }}</p>
        <p v-if="description" class="section-services__description">{{ description }}</p>
      </header>

      <div v-if="items.length === 0" class="section-services__empty orp-empty">
      ...
      <template v-else>
        <div v-if="viewMode === 'grid'" class="section-services__grid">
          <ServiceCard
            ...
            @details="openServiceModal"
          />
        </div>

        <div v-else-if="viewMode === 'carousel'" class="section-services__carousel-wrapper">
          <div class="section-services__carousel">
            <ServiceCard
              v-for="item in displayedItems"
              :key="item.id"
              :item="item"
        ...
        <div v-if="buttons && buttons.length" class="section-services__buttons orp-stack orp-stack--3">
          <a
...
defineOptions({ name: 'SectionServices' })
...
const showServiceModal = computed({
  get: () => selectedService.value !== null,
  set: (val) => { if (!val) selectedService.value = null }
})
