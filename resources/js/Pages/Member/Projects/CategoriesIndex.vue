<template>
  <MemberLayout>
    ...
    <BaseDataTable
      ref="dataTableRef"
      :endpoint="`/member/listings/${listing?.id}/project-categories`"
      :columns="columns"
    ...
    >
      ...
      <template #cell-projects_count="{ row }">
        <span class="badge bg-secondary">{{ row.projects_count || 0 }}</span>
      </template>
...
const columns = [
  ...
  { key: 'projects_count', label: 'Proyectos', sortable: true },
...
const createCategory = () => {
  ...
  router.post(`/member/listings/${listing.value.id}/project-categories`, formData, {
    onFinish: () => {
      sending.value = false
      closeModal()
...
const updateCategory = () => {
  ...
  formData.append('parent_id', form.parent_id ?? '')
  formData.append('is_active', form.is_active ? '1' : '0')
  router.put(`/member/listings/${listing.value.id}/project-categories/${editingCategory.value.id}`, formData, {
    onFinish: () => {
      sending.value = false
      closeModal()
      if (dataTableRef.value) {
...
const deleteCategory = (category) => {
  if (confirm(`Eliminar la categoria "${category.name}"?`)) {
    router.delete(`/member/listings/${listing.value.id}/project-categories/${category.id}`, {
      preserveScroll: true,
      onSuccess: () => {
        if (dataTableRef.value) {
          dataTableRef.value.reload()
