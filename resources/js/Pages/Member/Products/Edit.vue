...
const form = reactive({
  ...
  whatsapp_country: product.value.whatsapp_country || '+52',
...
const deleteProduct = () => {
  if (!confirm(`Eliminar el producto "${product.value?.name}"?`)) return
  router.delete(`/member/listings/${listing.value.id}/products/${product.value.id}`, {
    preserveScroll: true,
    onSuccess: () => {
