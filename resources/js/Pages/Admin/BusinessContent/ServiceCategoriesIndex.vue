<template>
  <AdminLayout>
    <Head :title="`Categorias de Servicios - ${listing?.name || ''}`" />
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
      <div>
        ...
        <h1 class="h4 mb-1 mt-1">{{ listing?.name || '' }} - Categorias de Servicios</h1>
      </div>
          ...
          <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
              ...
              <tr v-for="cat in categories" :key="cat.id">
                ...
                <td>
                  <span class="badge bg-light text-dark">{{ cat.services?.length || 0 }}</span>
                </td>
                <td>
...
const createCategory = () => {
  sending.value = true
  router.post(`/admin/listings/${listing.value.id}/service-categories`, form, {
    onFinish: () => {
      sending.value = false
      closeModal()
    },
...
const updateCategory = () => {
  sending.value = true
  router.put(`/admin/listings/${listing.value.id}/service-categories/${editingCategory.value.id}`, form, {
    onFinish: () => {
      sending.value = false
      closeModal()
    },
