...
<head>
    ...
    <title>{{ $title ?? 'Directorio de Negocios' }} - {{ config('app.name', 'Acerca.site') }}</title>
...
<body>
    ...
    <footer class="bg-dark text-white py-4 mt-5">
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <h5>{{ config('app.name', 'Acerca.site') }}</h5>
                    <p class="text-muted mb-0">Tu directorio de negocios locales</p>
                </div>
                <div class="col-md-6 text-md-end">
                    <p class="text-muted mb-0">&copy; {{ date('Y') }} {{ config('app.name', 'Acerca.site') }}. Todos los derechos reservados.</p>
                </div>
            </div>
        </div>
    </footer>
