...
{
    protected MinisiteExtensionRegistry $extensionRegistry;

    public function __construct(MinisiteExtensionRegistry $extensionRegistry)
    {
            ...
            ->map(function ($section) use ($listing) {
                ...
                switch ($section->section_type) {
                    ...
                    case 'restaurant_menu':
                        $provider = $this->extensionRegistry->getProvider('restaurant_menu');
                        $sectionData['items'] = $provider
                            ? $provider->getSectionData($listing, $config)
                            : [];
                        break;
                    case 'properties':
                        $provider = $this->extensionRegistry->getProvider('properties');
                        $sectionData['items'] = $provider
                            ? $provider->getSectionData($listing, $config)
                            : [];
                        break;
    ...
    public function menu(string $slug)
    {
        ...
        $provider = $this->extensionRegistry->getProvider('restaurant_menu');
        $menuData = $provider ? $provider->getSectionData($listing, []) : [];
    ...
    {
        return $this->renderPage($slug, 'properties', 'Propiedades', function ($b) {
            $provider = $this->extensionRegistry->getProvider('properties');
            return [
                'items' => $provider ? $provider->getSectionData($b, []) : [],
                'property_types' => $provider ? $provider->getPropertyTypes($b) : [],
            ];
        });
    }
    ...
    public function propertyDetail(string $slug, string $propertySlug)
    {
        $provider = $this->extensionRegistry->getProvider('properties');
        if (!$provider) {
            abort(404, 'Propiedades no disponibles');
        }
    ...
    protected function formatPropertyValue($value): mixed
    {
    ...
    private function getExistingSections(Listing $listing): array
    {
        ...
        foreach ($this->extensionRegistry->getAllProviders() as $key => $provider) {
