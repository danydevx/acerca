<template>
  <AdminLayout>
    <Head :title="`Turnos - ${listing.name}`" />
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
      <div>
        ...
        <h1 class="h4 mb-1 mt-1">{{ listing.name }} - Turnos</h1>
      </div>
      <div>
...
const createSlot = () => {
  ...
  router.post(`/admin/listings/${listing.value.id}/slots`, data, {
    onFinish: () => {
...
const updateSlot = () => {
  ...
  delete data.id
  router.put(`/admin/listings/${listing.value.id}/slots/${editingSlot.value.id}`, data, {
    onFinish: () => {
      saving.value = false
      closeEditModal()
    },
...
const toggleSlot = (slot) => {
  router.put(`/admin/listings/${listing.value.id}/slots/${slot.id}`, {
...
const deleteSlot = (slot) => {
  if (confirm('Eliminar este turno?')) {
    ...
    router.delete(`/admin/listings/${listing.value.id}/slots/${slot.id}`, {
      preserveScroll: true,
      onFinish: () => {
        deleting.value = null
      },
