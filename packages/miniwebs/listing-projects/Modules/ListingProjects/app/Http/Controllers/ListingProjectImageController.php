...
namespace Modules\ListingProjects\Http\Controllers;
...
class ListingProjectImageController extends Controller
{
    public function store(Request $request, Listing $listing, ListingProject $project)
    {
        $user = Auth::user();
        abort_unless($listing->user_id === $user->id || $user->hasAnyRole(['superadmin', 'admin']), 403);
        abort_unless($project->listing_id === $listing->id, 403);
        ...
        foreach ($files as $file) {
            ...
            if (!in_array($mimeType, ['image/jpeg', 'image/png']) && !in_array($extension, ['jpg', 'jpeg', 'png'])) {
    ...
    public function destroy(Listing $listing, ListingProject $project, ListingProjectImage $image)
    {
        $user = Auth::user();
        abort_unless($listing->user_id === $user->id || $user->hasAnyRole(['superadmin', 'admin']), 403);
        abort_unless($project->listing_id === $listing->id, 403);
        abort_unless($image->listing_project_id === $project->id, 403);
